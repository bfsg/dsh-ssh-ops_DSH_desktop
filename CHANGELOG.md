...
- **防 Agent 重试/绕行级联**：拦截改返回成功形状卡片后，Agent 不再因 tool error 反复重试（此前约 3 次/秒），也消除了"删不掉→核心目标完不成→自行下载 sshpass 绕行→刷屏"的级联诱因。`ssh_exec`/`sftp_delete`/`db_execute` 的卡片文案与 `ssh_write` 的拦截文案现在都显式标注「**尚未执行**」「**请勿重试本命令**」「**请勿改用 sshpass/其它工具或解释器绕行（仍会被拦）**」「删除由人工确认完成，可先推进不依赖该删除的步骤」。局限：黑名单为关键词级、非沙箱，`python -c "os.remove(...)"` 等不含关键词的调用理论上可绕过字符串匹配——靠卡片文案告诫 Agent 勿绕行兜底，执意绕行属 LLM 行为问题，非插件层能完全根治。
