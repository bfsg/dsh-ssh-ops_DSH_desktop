...
- **修复：部分被拦截命令不弹确认卡**。此前 `ssh_exec`/`sftp_delete` 只有在目标连接存在「存活终端会话」且命令可安全预填时才入队确认卡——没有开终端、命令含控制字符（Tab/换行等）或命令超长时会被拦下却静默丢弃卡片。现在只要连接存在就始终入队：有存活终端且命令可安全输入时走「终端模式」（执行=写入 PTY 并回车），否则走「exec 模式」（执行=人工批准后通过独立 exec 通道运行）。
- **修复：人工点击「执行」后 agent 仍以为被拦截**。确认结果此前只在面板内消化，agent 无法得知人工是否已执行/撤销。现在批准/撤销都会写入确认历史，并新增 agent 工具 `ssh_confirmations`：列出待确认与最近已处理记录（含 exit code、stdout/stderr、撤销/失败状态）。`ssh_exec`/`ssh_write`/`sftp_delete` 的拦截文案与工具说明也明确提示：操作员处理后调用 `ssh_confirmations` 查看结果，不要重试或绕行。
- **连接服务器弹窗：默认用户名可自定义**。此前用户名默认写死为 `paas`；现在用户名下拉右侧新增 ★ 按钮，选中任意已保存用户名（内置或自定义）后点 ★ 即设为默认，下拉里对应项显示「（默认）」，之后每次打开连接弹窗都会自动选中该默认用户（本机保存，删除该用户时自动回退为 `paas`）。
- **终端选中即复制**：在终端里用鼠标拖选文本、松手即自动写入系统剪贴板，右下角短暂提示「已复制 N 字符」（普通单击不会触发；剪贴板 API 不可用时自动回退到隐藏文本框复制）。
- **文件页按服务器记住浏览目录**：每台服务器最后成功浏览的目录会按连接记住，切换终端/服务器后再回文件页（或在文件页直接切换服务器）会回到该服务器上次所在的目录，而不是每次都回到根目录；若记忆的目录已被删除/无权访问，自动回退到 `/` 并清掉该记忆。
...
- **修复：确认卡执行/撤销在 Windows/cmd 终端上的残留粘连**。`ssh_write` 被拦的命令字符已流式写入远端行缓冲；`Ctrl-U`（`\x15`）只是 Unix 行清除，Windows cmd 不认——此前点「执行」会把残留文本与新命令拼成一条提交（命令粘连），点「撤销」后残留仍留在缓冲里（之后手动回车会执行一条未经确认的命令）。现执行与撤销都会发送 `\x15` + 与被拦命令等长的退格（`typedLength`），在 cmd 上逐字擦除缓冲，在 Unix PTY 上为无害空操作；同一会话同一命令仍只入队一张卡。
...
> DSH Desktop 桌面版 fork 的整理版：自 0.2.19 起的所有 fork 增强完整收录于此版（0.2.19 曾多次原地更新 Release，0.2.20 为统一快照）。基于上游 0.2.18。

- **DSH Desktop 桌面版界面适配**：检测无边框窗口（Windows `titleBarOverlay` 36px 标题栏 / macOS 拖拽区），SSH 面板顶端对齐侧栏「新会话」按钮上边沿，不与系统窗口按钮重叠。
- **多终端标签页**：每台服务器一个标签、标签右侧 `×` 单独断开、切换保留终端回看；删除「打开/关闭终端」按钮与右上角冗余 `＋`；面板顶部 `×` 仅隐藏面板；会话退出后标签可重开。
- **`ssh_write` 输入后回车 + 指定终端**：`press_enter`（默认 true，补 `\r` 回车，支持密码/`[Y/n]` 等 raw 提示）；`connection_id` 指定目标终端且无会话时自动开 PTY；`ssh_list` 输出 `[id: <connectionId>]` 供定位非活动终端。
- **连接对话框「保存并连接」**：临时表单可保存为 SSH 资源并连接（凭据写入本机 DSH 凭据库）；修复成功后残留错误、profileSave 取 profileId。
- **文件面板路径直达**：路径栏可点击编辑，输入远端绝对路径回车跳转（`..` 上一级、Esc 取消），随浏览自动同步。
- **文件面板拖拽上传**：从系统资源管理器把文件/文件夹拖进列表即上传（递归读目录、自动建远端目录）；拖到目录行上=上传进该子目录；拖入时虚线高亮与提示。
- **终端日志关键字着色**：输出写入前给行内独立等级关键字上色——`FATAL` 红粗体、`ERROR` 红、`WARN`/`WARNING` 黄、`INFO` 青、`DEBUG`/`TRACE` 灰，只染关键字词本身。
- **快捷命令栏**：SSH 面板终端页顶部可折叠「快捷命令」条（Xshell 式），支持分组与增删管理；点按钮把命令+回车发给当前活动终端（等同人工输入，受待确认危险命令守卫）。命令**持久化在 host 存储域**（`ssh_ops_quick_commands`），重启/端口变化不丢失（首次加载自动迁移旧的 localStorage 数据）；修复空列表时管理按钮被隐藏、无法添加第一条的问题。
- 附本地测试工具 `test-sshd.mjs` / `test-client.mjs`，与 `INSTALL.md` 安装说明。
