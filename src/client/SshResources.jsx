...
const styles = {
  page: { padding: "20px 2px", color: "var(--dsw-alias-label, #20242a)", maxWidth: 900 },
  pageHeader: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 18, marginBottom: 18 },
  heading: { margin: 0, fontSize: 18 }, description: { margin: "6px 0 0", fontSize: 13, color: "var(--dsw-alias-label-tertiary, #6d7685)", lineHeight: 1.5 },
  list: { display: "grid", gap: 10 }, card: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, padding: 14, border: "1px solid var(--dsw-alias-border, #d8dce3)", borderRadius: 10 },
  cardMain: { minWidth: 0 }, cardTitle: { fontWeight: 650, fontSize: 14, display: "flex", gap: 8, alignItems: "center" }, address: { marginTop: 4, fontSize: 13, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }, meta: { marginTop: 5, fontSize: 12, color: "var(--dsw-alias-label-tertiary, #6d7685)" },
  connected: { fontSize: 11, color: "#138a47", background: "#e6f7ed", padding: "2px 6px", borderRadius: 99 }, cardActions: { display: "flex", flexWrap: "wrap", justifyContent: "flex-end", gap: 7 },
  primary: { border: 0, borderRadius: 7, padding: "7px 11px", background: "var(--dsw-alias-brand, #2d6cdf)", color: "white", cursor: "pointer", fontSize: 13, whiteSpace: "nowrap" }, secondary: { border: "1px solid var(--dsw-alias-border, #b8bfca)", borderRadius: 7, padding: "6px 10px", background: "transparent", color: "inherit", cursor: "pointer", fontSize: 13, whiteSpace: "nowrap" }, danger: { border: 0, borderRadius: 7, padding: "6px 8px", background: "transparent", color: "#c62828", cursor: "pointer", fontSize: 13 },
  empty: { padding: 28, border: "1px dashed var(--dsw-alias-border, #cbd1da)", borderRadius: 10, color: "var(--dsw-alias-label-tertiary, #6d7685)", textAlign: "center" },
  groupPanel: { border: "1px solid var(--dsw-alias-border, #d8dce3)", borderRadius: 10, padding: 12, marginBottom: 16 }, groupTitle: { fontSize: 13, fontWeight: 650, marginBottom: 8 }, groupCreate: { display: "flex", gap: 8, maxWidth: 440 }, groupChips: { display: "flex", gap: 7, flexWrap: "wrap", marginTop: 10 }, groupChip: { display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 7px", borderRadius: 99, background: "var(--dsw-alias-fill, #eef1f5)", fontSize: 12 }, chipDelete: { border: 0, background: "transparent", color: "#9b2525", cursor: "pointer", padding: 0, fontSize: 15, lineHeight: 1 }, groupedList: { display: "grid", gap: 18 }, groupHeading: { margin: "0 0 8px", fontSize: 14 }, groupEmpty: { padding: 12, color: "var(--dsw-alias-label-tertiary, #6d7685)", border: "1px dashed var(--dsw-alias-border, #cbd1da)", borderRadius: 8, fontSize: 12 },
  backdrop: { position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,.42)", display: "flex", alignItems: "center", justifyContent: "center" }, dialog: { width: 440, maxWidth: "calc(100vw - 32px)", maxHeight: "calc(100vh - 32px)", overflow: "auto", background: "var(--dsw-alias-background, #fff)", color: "var(--dsw-alias-label, #20242a)", borderRadius: 12, padding: 18, boxShadow: "0 20px 60px rgba(0,0,0,.28)", display: "flex", flexDirection: "column", gap: 11 }, dialogTitle: { fontSize: 16, fontWeight: 650 },
  field: { display: "flex", flexDirection: "column", gap: 5, fontSize: 13 }, hint: { color: "var(--dsw-alias-label-tertiary, #6d7685)", fontWeight: 400 }, input: { width: "100%", boxSizing: "border-box", border: "1px solid var(--dsw-alias-border, #b8bfca)", borderRadius: 7, padding: "7px 8px", background: "transparent", color: "inherit", fontSize: 13 }, twoColumns: { display: "grid", gridTemplateColumns: "110px 1fr", gap: 10 }, check: { display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#a32626" }, actions: { display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 4 }, error: { padding: "8px 10px", borderRadius: 7, background: "rgba(204,45,45,.11)", color: "#b32222", fontSize: 13 }
};
